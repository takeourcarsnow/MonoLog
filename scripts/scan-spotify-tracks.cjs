...
async function main() {
  ...
  const uniqueTracks = tracks.filter(track => {
    if (seen.has(track.spotifyLink)) return false;
    seen.add(track.spotifyLink);
    return true;
  });

  function isValidSpotifyLink(link) {
    return link && (link.startsWith('https://open.spotify.com/') || link.startsWith('https://spotify.link/'));
  }

  const validTracks = uniqueTracks.filter(t => isValidSpotifyLink(t.spotifyLink));

  validTracks.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

  function pad(s, n) { return (s+'').padEnd(n).slice(0,n); }
  console.log('\nFound ' + validTracks.length + ' spotify links:\n');
