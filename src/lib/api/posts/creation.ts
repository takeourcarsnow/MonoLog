...
export async function createOrReplaceToday({ imageUrl, imageUrls, caption, alt, public: isPublic = true, spotifyLink, camera, lens, filmType }: { imageUrl?: string; imageUrls?: string[]; caption?: string; alt?: string; public?: boolean; spotifyLink?: string; camera?: string; lens?: string; filmType?: string }) {
  ...
  const res = await fetch('/api/posts/create', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token2 ? { Authorization: `Bearer ${token2}` } : {}) }, body: JSON.stringify({ imageUrls: finalUrls, thumbnailUrls: finalThumbUrls, caption, alt, public: isPublic, spotifyLink, camera, lens, filmType }) });
